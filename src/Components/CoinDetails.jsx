...
const CoinDetails = () => {
  ...
  useEffect(() => {
    const fetchCoin = async () => {
      try {
        const { data } = await axios.get(`https://api.coingecko.com/api/v3/coins/${params.id.toLowerCase()}`);
        setCoin(data);
        const {data:chartData} = await axios.get(`https://api.coingecko.com/api/v3/coins/${params.id.toLowerCase()}/market_chart?vs_currency=${currency}&days=${days}`)
...
const CustomBar = ({ high, low }) => (
  <VStack w={"full"}>
    <Progress value={50} colorScheme={"purple"} borderRadius={"10px"}  w={"full"} />
